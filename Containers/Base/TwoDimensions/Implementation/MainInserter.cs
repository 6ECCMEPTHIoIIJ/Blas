...
public class MainInserter<T> : 
    IElementInserter<IEnumerable<T>>
{
    private readonly IVectorBase<IVectorBase<T>> _data;

    public MainInserter(IVectorBase<IVectorBase<T>> data)
    {
        _data = data;
    }

    public void Insert(Int32 index, IEnumerable<T> element) => _data.Insert(index, new VectorBase<T>(element));
