...
public class RowedMatrix<T> : IRowedMatrix<T> where T : System.Numerics.INumber<T>
{
    private readonly IRowedMatrix<T> _implementation;

    #region Constructors

    public RowedMatrix() => _implementation = new RowedMatrixImplementation();

    public RowedMatrix(IEnumerable<IEnumerable<T>> enumerable) =>
        _implementation = new RowedMatrixImplementation(enumerable);

    public RowedMatrix(Int32 nRows, Int32 nColumns) => _implementation = new RowedMatrixImplementation(nRows, nColumns);
    ...
    public IColumnedMatrixBase<T> ToColumned() => _implementation.ToColumned();

    public IVectorBase<T> this[Int32 rowIndex]
    {
        get => _implementation[rowIndex];
        set => _implementation[rowIndex] = value;
    }

    public T this[Int32 rowIndex, Int32 columnIndex]
    {
        get => _implementation[rowIndex, columnIndex];
        set => _implementation[rowIndex, columnIndex] = value;
    }

    public T At(Int32 rowIndex, Int32 columnIndex) => _implementation.At(rowIndex, columnIndex);

    public IRowedMatrixBase<T> Set(Int32 rowIndex, Int32 columnIndex, T value) => _implementation.Set(rowIndex, columnIndex, value);

    public IRowedMatrixBase<T> Resize(Int32 nRows, Int32 nColumns) => _implementation.Resize(nRows, nColumns);

    public override String? ToString() => _implementation.ToString();

    public IMatrixBase<T> SetCellDataFormatter(Func<T, String>? cellDataFormatter) => _implementation.SetCellDataFormatter(cellDataFormatter);
    ...
    private class RowedMatrixImplementation : RowedMatrixBase<T>, IRowedMatrix<T>
    {
        private static readonly OutputFormatter _rowFormatter = new("[ ", " | ", " ]\n");

        private static readonly OutputFormatter _columnFormatter = new("[ ", " ]\n[ ", " ]\n");

        public RowedMatrixImplementation() : base(_rowFormatter, _columnFormatter)
        {
        }

        public RowedMatrixImplementation(IEnumerable<IEnumerable<T>> enumerable)
            : base(enumerable, _rowFormatter, _columnFormatter)
        {
        }

        public RowedMatrixImplementation(Int32 nRows, Int32 nColumns)
            : base(nRows, nColumns, _rowFormatter, _columnFormatter)
        {
        }
        ...
        public IMatrix<T>? ToReversed() => null;

        public new IColumnedMatrixBase<T> ToColumned() => new ColumnedMatrix<T>(this);
    }
